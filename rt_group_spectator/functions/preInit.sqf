// (local missionNamespace) Задержка между проверками видимых юнитов в spectator'е
RT_SPECTATOR_VAR_CHECKUNITS_DELAY = 5;
// (local player) Handler отключения spectator'а при смерти
RT_SPECTATOR_VAR_SPECTATOR_SET_HANDLER = "VARS_RT_SPECTATOR_VAR_SPECTATOR_SET_HANDLER";

// (local uiNamespace) Control с текстом Подсказки
RT_SPECTATOR_VAR_HINT_CTRL = "VARS_RT_SPECTATOR_VAR_HINT_CTRL";
// (local player) Handler нажатия Enter
RT_SPECTATOR_VAR_KEY_HANDLER = "VARS_RT_SPECTATOR_VAR_KEY_HANDLER";
// (local player) Handler отключения spectator'а при смерти
RT_SPECTATOR_VAR_KILLED_HANDLER = "VARS_RT_SPECTATOR_VAR_KILLED_HANDLER";
// (local player) дополнительный Handler отключения spectator'а при респавне
RT_SPECTATOR_VAR_RESPAWN_HANDLER = "VARS_RT_SPECTATOR_VAR_RESPAWN_HANDLER";
// (server missionNamespace) Handler на эвент ace_unconscious
RT_SPECTATOR_VAR_UNCOSCIOUS_HANDLER = "VARS_RT_SPECTATOR_VAR_UNCOSCIOUS_HANDLER";
// (local player) Handler проверки видимых юнитов в spectator'е
RT_SPECTATOR_VAR_CHECKUNITS_HANDLER = "VARS_RT_SPECTATOR_VAR_CHECKUNITS_HANDLER";

// Количество Цитат
RT_SPECTATOR_HINT_QUOTE_COUNT = 43;
RT_SPECTATOR_HINT_FUN_QUOTE_COUNT = 6;

RT_SPECTATOR_HINT_QUOTES_ADDED = [];
RT_SPECTATOR_HINT_QUOTES_WHITELIST = [];