...
// Stalking ended, if by the optional condition, then decide where to put the next WP
	...
	If ({alive _x} count (units _stalkedGroup) == 0) Then {["Stalked group %1 was eliminated",_stalkedGroup] call BIS_fnc_logFormat; _stalkerGroup setVariable ["BIS_Stalk",nil];};
