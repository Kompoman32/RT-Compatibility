...
AMA_medical_assistant_Display = 180025;
AMA_medical_assistant_Display_Main_Group = 956;

RT_Medical_Assitant_Init = {
    disableSerialization;

    //== No dialog, no display or is dedicated server/headless client
    if (isNull (findDisplay AMA_medical_assistant_Display) || isNull ((findDisplay AMA_medical_assistant_Display) displayCtrl AMA_medical_assistant_Display_Main_Group) || !hasInterface) exitWith {};

    private _display = (findDisplay AMA_medical_assistant_Display);
    private _mainCtrlGroup = ((findDisplay AMA_medical_assistant_Display) displayCtrl AMA_medical_assistant_Display_Main_Group);
    ...
    for "_i" from 0 to (18-1) do
    {
        ...
        private _ctrl_dir = _display ctrlCreate ["RscPicture", (100 + _i), _ctrl];
    ...
    for "_o" from 0 to 1 step 0 do
    {
            private _unitsList = (units group player);
            {
                [_x] call {
                    ...
                    if (((isNull _ctrl_dir) && (isNull (findDisplay AMA_medical_assistant_Display)))) exitWith {};
            ...
            if (isNull (findDisplay AMA_medical_assistant_Display)) exitWith { _o = 2 };
